...
## 2026-06-08 - Missing Rate Limit on Invite Redemption
**Vulnerability:** The `/rauthredeem` endpoint in `internal/handlers/invite.go` was missing rate limiting, allowing potential attackers to brute force invite tokens or spam account creation requests.
**Learning:** While other endpoints like login and registration had rate limits, the invite redemption logic, which creates new users, lacked IP-based throttling. Furthermore, testing these endpoints requires explicitly mocking `c.Request().RemoteAddr` and setting configuration thresholds in test setups to ensure `c.RealIP()` rate limiting logic doesn't fail default tests.
**Prevention:** Always verify that any endpoint which handles sensitive operations (especially account creation, login, or token redemption) is enveloped by the `core.CheckRateLimit` utility.
