...
## 2026-06-03 - Intercepting slog logs in Tests
**Learning:** Testing log outputs generated by `slog` requires setting a custom default logger using `slog.SetDefault` with a buffer-backed handler (like `slog.NewJSONHandler`).
**Action:** Always intercept and check logs for error or warning pathways to achieve comprehensive test coverage, especially when functions return `nil` in error states but log the issue.
