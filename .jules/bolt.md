...
## 2026-06-12 - Optimizing Pipeline Command Storage
**Learning:** Storing pipeline command responses (like `*redis.StringCmd`) in a `map[string]` inside a loop forces memory allocations and map hash lookups upon retrieval. Using a simple slice `[]*redis.StringCmd` pre-allocated to the exact `len` and indexing it via the loop's `i` index avoids map overhead entirely and provides a measurable speedup for batch operations.
**Action:** When iterating over a slice of keys to populate a Redis pipeline and store the resulting command objects, store them in a slice `[]*CommandType` rather than a `map[string]*CommandType`. Note: Some cases that aggregate tokens from multiple keys might still require a map for deduplication or secondary lookup.
