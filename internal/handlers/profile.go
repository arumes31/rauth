...
func (h *ProfileHandler) RevokePasskey(c echo.Context) error {
	...
	if userRecord.Email != "" {
		...
		go core.Send2FAModifiedNotification(core.TwoFactorNotificationOptions{Email: userRecord.Email, Username: username, Action: "Removed (Passkey)", IP: c.RealIP(), Device: device})
	}
