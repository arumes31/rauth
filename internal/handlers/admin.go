...
func (h *AdminHandler) Dashboard(c echo.Context) error {
	...
	for {
		var batch []string
		var err error
		// ⚡ Bolt optimization: Batch Scan requests to avoid numerous small network round-trips
		batch, cursor, err = core.TokenDB.Scan(core.Ctx, cursor, "X-rauth-authtoken=*", 1000).Result()
		if err != nil {
